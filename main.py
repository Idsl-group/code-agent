...
def coding_agent(query: str) -> str:
    ...
    PARAMETERS:
        query (str):
            ...
            - Constraints, style requirements, or standards
    ...
    EXAMPLE TOOL CALL (JSON):
        {
            "query": "Programming Language: Python 3.11\\n\\nObjective:\\nImplement a small, production-ready library for computing and caching Fibonacci numbers.\\n\\nFunctional Requirements:\\n1. Create a module named `fibonacci.py`.\\n2. Implement the following public functions:\\n   - fib(n: int) -> int\\n   - fib_sequence(n: int) -> list[int]\\n\\nFunction Specifications:\\n- fib(n):\\n  - Returns the nth Fibonacci number.\\n  - Must raise ValueError if n < 0.\\n  - Must run in O(n) time using dynamic programming.\\n\\n- fib_sequence(n):\\n  - Returns a list containing Fibonacci numbers from 0 through n inclusive.\\n  - Must internally reuse fib() or a shared cache.\\n  - Must raise ValueError if n < 0.\\n\\nCaching Requirements:\\n- Use an in-memory cache to avoid recomputing values.\\n- Cache must persist across multiple function calls within the same process.\\n- Do NOT use functools.lru_cache.\\n\\nCode Quality & Style:\\n- Follow PEP 8 strictly.\\n- Use type hints for all public functions.\\n- Include concise docstrings for the module and each function.\\n- No global mutable state except the cache.\\n\\nError Handling:\\n- Validate input types and values.\\n- Provide clear error messages.\\n\\nTesting Requirements:\\n- Include a __main__ block that demonstrates usage with sample inputs.\\n- Example outputs should be printed for n = 5 and n = 10.\\n\\nConstraints:\\n- Do not use recursion.\\n- Do not import external libraries.\\n- Code must be self-contained in a single file.\\n\\nDeliverable:\\n- Final output should be the complete contents of fibonacci.py, ready to be written to disk."
        }
    """
    ...
    if tool_calls:=getattr(final_message, "tool_calls", None):
        ...
        final_answer = "\n\n".join([f.content for f in final_state["messages"] if f.type=="ai"]) #todo
